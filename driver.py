# -*- coding: utf-8 -*-
"""
Modulo de comunicação com Whatsapp
...
from storage import DriversStorage
		...
		if driver == None:
			...
			for c in chats_objs:
				obj = {
					...
					"unreads": len(driver["obj"].get_unread_messages_in_chat(c.id, include_me=True, include_notifications=True)),
		...
		if driver == None:
			...
			while retry == True:
				try:
					...
					messages_ids = driver["obj"].get_all_message_ids_in_chat(chat, include_me=True)
					retry = False
				except:
					continue
	...
	def reg_sent(self, user, chat_id):
		...
		try:
			...
			if i >= 0:
				...
				if msg.sender.get_safe_name() == "You" or msg.sender.get_safe_name() == "Voc":
